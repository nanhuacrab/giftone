<?php /* Smarty version 2.6.26, created on 2015-01-29 18:21:08
         compiled from admin/admin_descclass.htm */ ?>
...
function save_industry(){
	...
	$.post("index.php?m=desc_class&c=add",{add_name:add_name,add_sort:add_sort,pytoken:pytoken},function(msg){
		if(msg==1){ 
			parent.layer.msg('类别名称不能为空！', 2, 8);return false;
		}else if(msg==2){
...
function subsort(id){
	...
	var pytoken=$("#pytoken").val(); 
	$.post("index.php?m=desc_class&c=ajax",{id:id,sort:sort,pytoken:pytoken},function(data){ 
		$("#sort"+id).html(sort);
		$("#sort"+id).show();
		$("#input"+id).hide();
...
function subname(id){
	...
	var pytoken=$("#pytoken").val();
	$.post("index.php?m=desc_class&c=ajax",{id:id,name:name,pytoken:pytoken},function(data){ 
		$("#name"+id).html(name);
		$("#name"+id).show();
		$("#inputname"+id).hide();
		if(data!=1){config_msg(data);}else{location.reload();}
...
<span class="admin_title_span">单页面类别管理</span>
<a  href="javascript:void(0)" onClick="add_class('添加类别','300','140','#houtai_div','')" class="admin_infoboxp_tj">添加类别</a>
</div>
...
<table width="100%" id="table_industry">
	...
	<tr class="admin_table_top">
		...
		<th width="60">编号</th>
		<th>类别名称<span class="clickup">(点击修改)</span></th>
		<th>排序（越大越靠前）</th>
...
');" ></td>
        ...
        <a href="javascript:void(0)" onClick="layer_del('确定要删除？', 'index.php?m=desc_class&c=del&delid=<?php echo $this->_tpl_vars['v']['id']; ?>
